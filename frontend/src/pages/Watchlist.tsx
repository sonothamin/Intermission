    ...
    <div className="space-y-6 flex flex-col h-full">
      <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-theme-secondary text-sm">Prioritize what to watch next.</p>
        </div>
            ...
            {viewMode === "list" && (
              <div className="flex-1 overflow-auto p-4 space-y-3">
                {filteredItems.map((item, index) => (
                  <div key={item.id} className="flex gap-4 p-3 bg-theme-secondary border border-theme rounded-lg group hover:border-theme-focus transition-colors">
                    
                    {/* Priority Controls */}
                    <div className="flex flex-col items-center justify-center gap-1 w-8">
                      <button 
                        onClick={() => handlePriorityChange(item.id, item.priority, 1)}
                        className="p-1 text-theme-muted hover:text-[#10b981] disabled:opacity-30"
                        disabled={index === 0 && item.priority >= 100}
                      >
                        <ArrowUp className="w-5 h-5" />
                      </button>
                      <span className="text-xs font-mono font-bold text-theme-primary">{item.priority}</span>
                      <button 
                        onClick={() => handlePriorityChange(item.id, item.priority, -1)}
                        className="p-1 text-theme-muted hover:text-red-500 disabled:opacity-30"
                      >
                    ...
                    <Link
                      ...
                      className="w-16 h-24 bg-theme-tertiary rounded overflow-hidden flex-shrink-0 hover:ring-2 hover:ring-[#10b981]/50 transition-all"
                    >
                      {item.poster_url ? (
                        <img src={item.poster_url} alt={item.title} className="w-full h-full object-cover" />
                      ) : (
                        <Film className="w-6 h-6 m-auto mt-9 text-theme-muted" />
                      )}
                    </Link>
                    ...
                    <div className="flex-1 py-1 flex flex-col min-w-0">
                      <Link
                        to={mediaPath(item.media_type, item.tmdb_id)}
                        className="font-semibold text-theme-primary text-lg leading-tight hover:text-[#10b981] transition-colors truncate"
                      >
                        {item.title}
                      </Link>
                      <div className="flex gap-2 mt-1">
                        ...
                        <span className="text-xs text-theme-secondary">{item.release_year || "Unknown"}</span>
                        {item.content_rating && <span className="text-xs text-theme-secondary border border-theme px-1 rounded">{item.content_rating}</span>}
                      </div>
                      
                      {item.notes && (
                        <p className="text-sm text-theme-secondary mt-2 italic flex-1">"{item.notes}"</p>
                      )}
                    </div>

                    {/* Actions */}
                    <div className="flex flex-col justify-between items-end gap-2 border-l border-theme pl-4">
                      <button 
                  ...
                  {filteredItems.map((item, index) => (
                    <div key={item.id} className="relative group rounded-lg overflow-hidden bg-theme-secondary border border-theme hover:border-theme-focus transition-colors flex flex-col h-full">
                      <Link to={mediaPath(item.media_type, item.tmdb_id)} className="block aspect-[2/3] bg-theme-tertiary relative">
                        ...
                        <div className="absolute top-2 left-2 bg-black/60 backdrop-blur rounded px-1.5 py-0.5 text-xs font-mono font-bold text-theme-primary">
                          {item.priority}
                        </div>
                      </Link>
                      <div className="p-3 flex-1 flex flex-col justify-between gap-3">
                        ...
                        <div className="flex items-center justify-between border-t border-theme pt-2">
                          <div className="flex gap-1">
                            <button 
                              onClick={() => handlePriorityChange(item.id, item.priority, 1)}
                              className="p-1 text-theme-muted hover:text-[#10b981] hover:bg-[#10b981]/10 rounded disabled:opacity-30"
                              disabled={index === 0 && item.priority >= 100}
                            ...
                            <button 
                              onClick={() => handlePriorityChange(item.id, item.priority, -1)}
                              className="p-1 text-theme-muted hover:text-red-500 hover:bg-red-500/10 rounded disabled:opacity-30"
                          ...
                          <div className="flex gap-1">
                            <button 
                              onClick={() => handleMoveToLibrary(item)}
                              className="p-1 text-theme-muted hover:text-[#10b981] hover:bg-[#10b981]/10 rounded"
                            ...
                            <button 
                              onClick={() => handleRemove(item.id)}
                              className="p-1 text-theme-muted hover:text-red-500 hover:bg-red-500/10 rounded"
                              title="Remove from watchlist"
                            >
                              <Trash2 className="w-4 h-4" />
                            </button>
